...
def set_playlist():
    ...
    # pylint: disable=W0718
        ...
        flash(
            "Kein Aktives Spotify Gerät gefunden. Bitte spiele irgendetwas auf deinem Spotify ab, und versuche es erneut.",
            "error",
        )
        print(e)
...
def skip():
    try:
        ...
        track_id_before_skip = current_playback["item"]["id"]

    except Exception as e:
        flash("Fehler beim Abrufen der aktuellen Wiedergabe: " + str(e), "error")
        return render_template("index.html", user_id=session["user_id"])
    try:
        next_track = trainer.get_next_track(
            session["playlist_id"], session["user_id"], track_id_before_skip
        )

    except Exception as e:
        flash("Fehler beim Abrufen des nächsten Tracks: " + str(e), "error")
        return render_template("index.html", user_id=session["user_id"])
    try:
        sp.start_playback(uris=[f"spotify:track:{next_track}"])
        if track_id_before_skip == next_track:
            print("Error: Gleicher Track nach Skip: " + track_id_before_skip)

    # pylint: disable=W0718
    except Exception as e:
        flash(str(e), "Spotify Error")
        print(e)
    return render_template("index.html", user_id=session["user_id"])
...
def check_guess():
    ...
    artists = ", ".join(
        track_data["artists"]
    )  # Alle Künstlernamen zu einem String verbinden
    ...
    return render_template(
        "index.html",
        year_guess=(guess_year + " (" + str(year) + ")"),
...
def stats():
    total_songs = trainer.get_active_track_count(
        session["playlist_id"], request.form.get("user_id")
    )
    known_songs = trainer.get_finished_track_count(
        session["playlist_id"], request.form.get("user_id")
    )
    total_trys = trainer.get_try_count(
        session["playlist_id"], request.form.get("user_id")
    )
    return render_template(
        "stats.html", total=total_songs, known=known_songs, trys=total_trys
    )
